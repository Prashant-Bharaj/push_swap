...
/*   Updated: 2025/12/21 17:59:17 by prasingh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int						is_in_lis(t_stack_node *node, int *lis_arr,
							int lis_size);
...
void					compute_dp_table(int *arr, int *dp, int *prev,
							int size);

int						*build_lis_array(int *arr, int *prev, int max_idx,
							int lis_size);

int						find_target_position(t_stack *a, int value);

int						calculate_cost(t_stack_node *node_b, t_stack *a,
							t_stack *b);
...
void					prepare_and_insert_element(t_data *data,
							t_stack_node *cheapest);

int						calculate_rotation_count(int pos, int size);

void					execute_rotation_operations(t_data *data, int rot_a,
							int rot_b);
